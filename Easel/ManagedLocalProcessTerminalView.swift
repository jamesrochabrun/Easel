...
private let logger = Logger(subsystem: "com.easel", category: "Terminal")
...
open class ManagedLocalProcessTerminalView: TerminalView, TerminalViewDelegate, LocalProcessDelegate {
  ...
  /// Called with raw terminal output bytes before they are fed to the terminal buffer.
  public var onDataReceived: ((ArraySlice<UInt8>) -> Void)?
  ...
  public func terminateProcessTree(graceSeconds: TimeInterval = 1.0) {
    ...
    DispatchQueue.global(qos: .utility).asyncAfter(deadline: .now() + graceSeconds) { [weak self] in
      ...
      logger.warning("Process group PID=\(pid) still alive; sending SIGKILL")
